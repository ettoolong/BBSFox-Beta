...
BBSImgurPicLoader.prototype={
  ...
  query: function(url, callback){
    ...
    {
      if(callback) {
        var xmlhttp = new this.Xhr(false, {UniUrl: url, LoaderAction: "query", callback: callback}, this.onPageResponse.bind(this));
        xmlhttp.open(url);
      }
      return true;
    }
  ...
  onPageResponse: function(info) {
    ...
    if(info.status == 200)
    {
      ...
      if(mainimg)
      {
        ...
        {
          this.listener.cb_show(info.owner, 'https:' + mainimg.src);
        }
        else if(info.LoaderAction=='query')
        {
          info.callback('https:' + mainimg.src);
        }
